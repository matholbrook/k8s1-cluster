...
ssh-keygen -f '/home/mat/.ssh/known_hosts' -R 'k8s1-vm1'
ssh-keygen -f '/home/mat/.ssh/known_hosts' -R 'k8s1-vm2'
ssh-keygen -f '/home/mat/.ssh/known_hosts' -R 'k8s1-vm3'
```
...
```bash
cd ../ansible
ansible-playbook -i k8s1-hosts.ini ../../../ansible/apt.yaml
```

## Set up Kubernetes

```bash
ansible-playbook -i k8s1-hosts.ini ../../../ansible/k8s.yaml
```

## Create k8s Cluster

```bash
ansible-playbook -i ../ansible/k8s1-hosts.ini ../ansible/playbooks/ansible-playbook_k8s-cluster-calico.yaml
```
...
```bash
ssh-keygen -f '/home/mat/.ssh/known_hosts' -R 'k8s1-vm1'
ssh mat@k8s1-vm1 "cat ~/.kube/config" > ~/.kube/k8s1-config
...
```bash
kubectl label no k8s1-vm2 node-role.kubernetes.io/worker=worker
kubectl label no k8s1-vm3 node-role.kubernetes.io/worker=worker
```
